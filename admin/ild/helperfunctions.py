...
def createRepository( owner, repository, description ):
  """ create a new repository """
  creaDict = dict( name=repository, description=description, private=False, allow_squash_merge=False, allow_merge_commit=False )

  result = curl2Json(requestType="POST", parameterDict=creaDict, url="https://api.github.com/orgs/%s/repos" % owner)
...
def cloneAndPush(  owner, repository, newOwner, newRepository, force=False ):
  ...
  command = ["git", "clone", "https://github.com/%s/%s.git" % (owner, repository), directory]
